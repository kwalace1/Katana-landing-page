...
export async function submitWaitlist(payload) {
  ...
  try {
    emailSent = await notifyByEmail({
      subject: `New ${productLabel} waitlist signup: ${email}`,
      text: `New ${productLabel} waitlist signup\n\nEmail: ${email}\nProduct: ${productLabel}\nTime: ${when}\n`,
      html: `<h2>New ${escapeHtml(productLabel)} waitlist signup</h2><p><strong>Email:</strong> ${escapeHtml(email)}</p><p><strong>Product:</strong> ${escapeHtml(productLabel)}</p><p><strong>Time:</strong> ${when}</p>`,
    });
  } catch (error) {
